...
public interface CookieRepository extends JpaRepository<CookieEntity, Long> {
    ...
    @Modifying
    @Query(value = "UPDATE TPBANKOWNER.REGISTRATION_COOKIES_CLUB_WIFI SET RCCW_CODE = :code, RCCW_ADDRESS = :address, RCCW_SUBSCRIBER = :subscriber, RCCW_CREATION_DATE = :creationDate, RCCW_FIRST_COOKIE = :firstCookie, RCCW_SECOND_COOKIE = :secondCookie WHERE RCCW_MAC = :mac", nativeQuery = true)
    ...
    @Modifying
    @Query(value = "INSERT INTO TPBANKOWNER.REGISTRATION_COOKIES_CLUB_WIFI(RCCW_IDENTIFIER, RCCW_MAC, RCCW_CODE, RCCW_ADDRESS, RCCW_SUBSCRIBER, RCCW_FIRST_COOKIE, RCCW_SECOND_COOKIE, RCCW_CREATION_DATE) VALUES(:identifier, :mac, :code, :address, :subscriber, :firstCookie, :secondCookie, :creationDate)", nativeQuery = true)
    ...
    @Query(value = "SELECT MAX(RCCW_IDENTIFIER) FROM TPBANKOWNER.REGISTRATION_COOKIES_CLUB_WIFI", nativeQuery = true)
    Long maxValue();
